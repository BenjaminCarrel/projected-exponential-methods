"""
Global error and performance of the DLRA methods for the Lyapunov ODE.
...
methods_styles += ['-x']
## Projected exponential Runge (non-strict)
dlra_solvers += ['PERK']
methods_kwargs += [{'order': 2, 'krylov_kwargs': {'size': 1, 'kind': 'extended', 'is_symmetric': True, 'invA': invA, 'invB': invB}, 'strict_order_conditions': False}]
methods_labels += ['Proj. exponential Runge (non strict)']
methods_styles += ['-p']
...
# Number of steps
nb_steps = np.logspace(1, 5, 10, dtype=int)
