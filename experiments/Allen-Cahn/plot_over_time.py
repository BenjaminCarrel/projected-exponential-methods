...
ref_sol = solve_matrix_ivp(ode, t_span, X0, solver="scipy", t_eval=ts, monitor=True, scipy_method='RK45')
Xs_ref = ref_sol.todense()
# print('Time taken by the reference solver (scipy LSODA with tol=1e-8): {:.2f} s'.format(ref_sol.computation_time))

#%% Plot at different times
time_to_plot = [0, 3, 5, 7, 10]
nb_plots = len(time_to_plot)
fig, axs = plt.subplots(1, nb_plots, figsize=(22, 4), sharex=True, sharey=True)
# fig.suptitle(f'Reference solution')
...
for i, t in enumerate(time_to_plot):
    ...
    axs[i].set_xticks([0, np.pi, 2*np.pi], [0, r'$\pi$', r'$2\pi$'])
    axs[i].set_yticks([0, np.pi, 2*np.pi], [0, r'$\pi$', r'$2\pi$'])
...
fig.savefig(f'figures/{ode_name}_reference_solution_over_time_{timestamp}.pdf', bbox_inches='tight')
...
#%% Plot at different times
fig, axs = plt.subplots(1, nb_plots, figsize=(22, 4), sharex=True, sharey=True)
# fig.suptitle(fr'Proj. exp. Euler ($r={rank}$)')
...
# Save it with timestamp
timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
fig.savefig(f'figures/{ode_name}_perk{order}_solution_over_time_{timestamp}.pdf', bbox_inches='tight')
...
fig, axs = plt.subplots(1, nb_plots, figsize=(22, 4), sharex=True, sharey=True)
# fig.suptitle(f'Predictive error')
...
timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
fig.savefig(f'figures/{X0.shape}_error_over_time_{timestamp}.pdf', bbox_inches='tight')
